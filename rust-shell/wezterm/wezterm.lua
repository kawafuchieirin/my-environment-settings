-- =============================================================================
-- WezTerm 設定ファイル (wezterm.lua)
-- =============================================================================
--
-- 概要:
--   WezTermターミナルエミュレータの設定ファイル。外観、フォント、
--   キーバインド、透過度などをカスタマイズ。
--
-- 配置先:
--   ~/.config/wezterm/wezterm.lua（シンボリックリンク）
--
-- 主な設定:
--   - カラースキーム: Dracula
--   - フォント: Menlo + Hack Nerd Font Mono
--   - 透過度: 92%（動的調整可能）
--   - GPU: WebGpu（高速描画）
--   - シェル: Nushell（/opt/homebrew/bin/nu --login）
--
-- キーバインド:
--   タブ操作:
--     Cmd + T/D       : 新しいタブ
...
--     Cmd + K         : 画面クリア
--     Cmd + F         : 検索
--     Cmd + +/-/0     : フォントサイズ変更/リセット
--
-- カスタマイズ:
--   - カラースキーム: config.color_scheme を変更
--   - フォント: config.font を変更
--   - キーバインド: config.keys テーブルに追加
--
-- 注意:
--   このファイルを編集して保存すると、WezTermを再起動しなくても
--   自動で設定が反映されます。
--
-- 参考:
--   help-wezterm コマンドでヘルプ表示
