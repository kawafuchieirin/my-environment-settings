#!/bin/bash
# =============================================================================
# setup.sh - macOS環境マスターセットアップスクリプト
# =============================================================================
#
# 概要:
#   新しいMacに開発環境をセットアップするためのマスタースクリプト。
#   対話モードまたはコマンドラインオプションで各種設定を選択可能。
#
# 使用方法:
#   ./setup.sh                     # 対話モードで実行
#   ./setup.sh --dry-run --all     # ドライランで全設定を確認
#   ./setup.sh --zsh --macos       # zsh環境とmacOS設定をセットアップ
#   ./setup.sh --nushell --vscode  # Nushell環境とVS Codeをセットアップ
#
# オプション:
#   -h, --help     : ヘルプを表示
...
#
# シェル環境の選択:
#   1) zsh + モダンCLIツール（従来環境）
#      - terminal/setup.sh を実行
#      - デフォルトシェルはzshのまま
#
#   2) Nushell + Rust製ツール（モダン環境）
#      - rust-shell/setup.sh を実行
#      - デフォルトシェルがNushellに変更される
#
# 前提条件:
#   - macOS（Darwin）
